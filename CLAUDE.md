...
- **Carrinho**: para visitantes sem sessão, guardado em `localStorage` (comportamento original do Passo 5). Para clientes autenticados (`role: "CUSTOMER"`), guardado na BD (`CartItem`, `@@unique([customerId, productId])`) e sincronizado entre dispositivos — mudança pedida fora da sequência de passos. `app/(loja)/layout.tsx` chama `auth()` e passa `isCustomer` como prop ao `CartProvider` (`hooks/use-cart.tsx`); o próprio hook decide ler/escrever em `localStorage` ou nas server actions de `lib/cart-actions.ts` consoante essa prop — nunca os dois ao mesmo tempo. Ao iniciar sessão (prop muda de `false` para `true`, incluindo no primeiro render se já vier autenticado), o carrinho local (se existir) é enviado para `mergeLocalCartIntoServer` — soma quantidades ao carrinho já guardado, respeitando sempre o stock atual do produto — e o `localStorage` é limpo a seguir. Todas as quantidades são sempre recalculadas a partir do `Product.stock` atual no servidor (nunca confiar no valor vindo do cliente), mesmo padrão de segurança já usado no checkout.
...
- **Envio**: taxa fixa de €2,50 se o subtotal do carrinho for inferior a €10, grátis a partir daí (`lib/shipping.ts`, `calculateShipping`/`FREE_SHIPPING_THRESHOLD`/`SHIPPING_FEE` — constantes partilhadas entre o cálculo de exibição no `/checkout` e o valor cobrado a sério). Cobrado via `shipping_options`/`shipping_rate_data` do próprio Checkout Session da Stripe (não é um line item de produto à parte), para aparecer corretamente destacado também na página da Stripe. `Order.shippingAmount` guarda o valor cobrado (0 se grátis); `Order.total` passa a vir de `session.amount_total` (valor real cobrado, já inclui envio) em vez de recalculado só a partir dos itens — mostrado como linha "Envio" separada nas páginas de detalhe da encomenda (admin e cliente) e nos emails de confirmação/aviso, para não haver descontinuidade entre a soma dos produtos e o Total apresentado. Não entra no cálculo de comissão de fornecedores (`OrderItem.commissionAmount`) — o envio é sempre 100% receita do admin.
...
- **Rotas**: `/torna-te-vendedor` (candidatura pública, dentro de `app/(loja)/` para ter Navbar/Footer) · `/fornecedor/login` · `/fornecedor/(painel)/painel` (dashboard) · `/fornecedor/(painel)/produtos` (CRUD só dos produtos próprios, reutiliza `ProductForm`) · `/fornecedor/(painel)/saldo` (saldo + histórico + pedir levantamento) · `/admin/candidaturas` (candidaturas de fornecedor, filtro de estado Pendente/Aprovado/Rejeitado — por omissão mostra Pendente+Rejeitado) · `/admin/fornecedores` (só fornecedores APPROVED) · `/admin/fornecedores/[id]` (detalhe: estatísticas — nº encomendas, unidades vendidas, total bruto/líquido —, lista de produtos e lista de encomendas com o estado de cada uma) · `/admin/levantamentos` (pedidos de levantamento, filtro de estado Pendente/Pago, separado de `admin/fornecedores`). Candidaturas, fornecedores aprovados e levantamentos foram separados em páginas/nav própria a pedido do utilizador, para não misturar "por decidir" com "já ativos" com "pedidos de pagamento". Middleware protege `/fornecedor/:path*` tal como já protegia `/admin/:path*`.
- **Badges de notificação nas sidebars**: `admin/(painel)/layout.tsx` mostra contagens em Produtos (aprovação pendente), Encomendas (estado PAID — recém-pagas, ainda por processar), Candidaturas (PENDING) e Levantamentos (PENDING); `fornecedor/(painel)/layout.tsx` mostra em "Painel" o nº de encomendas PAID com produtos desse fornecedor. Não é um sistema de "não lidas" (sem campo de leitura na BD) — é sempre a contagem atual da fila de itens que precisam de ação, recalculada a cada render do layout.
...
## Recuperação de password (cliente e fornecedor)

Construído fora da sequência de passos, a pedido direto do utilizador ("máxima segurança" + poder recuperar password). Cobre `Customer` e `Vendor` — não o admin (`User`), que não foi pedido e continua só com login direto.

- **Modelos**: `Customer.resetToken`/`resetTokenExpiresAt` e `Vendor.resetToken`/`resetTokenExpiresAt` (ambos opcionais, `resetToken` com `@unique`). Token gerado em `lib/tokens.ts` (`generateResetToken`, 32 bytes aleatórios via `crypto.randomBytes`, não é o JWT de sessão), validade de 1h (`RESET_TOKEN_TTL_MS`).
- **Rotas**: `/conta/recuperar` e `/fornecedor/recuperar` (pedir o link, form só com email) · `/conta/redefinir/[token]` e `/fornecedor/redefinir/[token]` (escolher password nova). Adicionadas à lista de páginas públicas em `auth.config.ts` (senão o middleware bloqueava por não haver sessão). Link "Esqueceste-te da password?" nos dois logins (`/conta/login`, `/fornecedor/login`).
- **Componentes partilhados**: `components/ForgotPasswordForm.tsx` e `components/ResetPasswordForm.tsx`, usados pelas 4 páginas (recebem a server action correspondente via prop, mesmo padrão do `DeleteProductButton`/`onDelete`). Os `page.tsx` de `redefinir/[token]` são Server Components que fecham o `token` do URL numa server action inline (`"use server"` dentro da função), evitando expor o token como estado extra no cliente.
- **Anti-enumeração de contas**: `requestCustomerPasswordReset`/`requestVendorPasswordReset` respondem sempre da mesma forma (mensagem genérica no formulário) quer o email exista ou não na BD — só geram e enviam o token se a conta existir de facto, mas nunca revelam isso ao utilizador.
- **Cabeçalhos de segurança**: `next.config.ts` (`headers()`) define `X-Frame-Options`, `X-Content-Type-Options`, `Referrer-Policy`, `Strict-Transport-Security` e `Permissions-Policy` para todas as rotas.
